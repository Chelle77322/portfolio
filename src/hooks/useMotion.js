...
/*Setting the origin the transition */
    ...
    function withTransition(callback){
        return () => {
            ...
            if(!store.scenes[sceneName]){
                console.warn(`${sceneName} is not registered, you might have misspelled the MotionScene name`);
                callback();
                return;
            }
            if(!store.onExit) {
            ...
            dispatch({
                type:actions.view.setExitView,sceneName,
            });
